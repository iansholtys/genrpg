...
usersRouter.get("/users", asyncRoute(async (req, res) => {
  const users = (await UserStorage.global().list()).map((entity) => entity.toJSON());
  res.json({ users });
}));

usersRouter.put("/users/:guid/admin", requireAdmin, asyncRoute(async (req, res) => {
...
usersRouter.delete("/users/:guid", requireAdmin, asyncRoute(async (req, res) => {
  const targetGuid = req.params.guid;
