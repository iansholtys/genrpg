...
genrpgApi.get("/permissions", async (req, res, next) => {
  try {
    const result = await pool.query(
...
genrpgApi.post("/roles", requireAdmin, async (req, res, next) => {
  try {
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
    const description =
      typeof req.body.description === "string" ? req.body.description.trim() : "";
...
genrpgApi.put("/roles/:id", requireAdmin, async (req, res, next) => {
  try {
...
genrpgApi.delete("/roles/:id", requireAdmin, async (req, res, next) => {
  try {
    const roleId = Number(req.params.id);
