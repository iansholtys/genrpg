...
function normalizeExtensionFieldSpec(property, rawSpec, machineName, label) {
...
/**
 * Package extension field declarations keyed by entity, then package machine name.
 * @returns {Map<string, Map<string, Record<string, object>>>}
 */
async function buildExtensionFields(packages) {
  const extensionFields = new Map();
  ...
  for (const pkg of packages) {
    ...
    for (const entry of manifest.extensions) {
      ...
      if (!extensionFields.has(entry.entity)) {
        extensionFields.set(entry.entity, new Map());
      }
      const byPackage = extensionFields.get(entry.entity);
      const specs = {};

      for (const [property, rawSpec] of Object.entries(rawFields)) {
        specs[property] = normalizeExtensionFieldSpec(property, rawSpec, pkg.machineName, moduleLabel);
      }
...
function getExtensionFields() {
  return packageCache?.extensionFields ?? new Map();
}

/**
 * Merge extension field specs for an entity and instance package set.
 * @returns {Record<string, object>}
 */
function mergeExtensionFieldSpecs(entityKey, packageNames, coreFieldKeys = []) {
  const byPackage = getExtensionFields().get(entityKey);
  if (!byPackage) {
    return {};
  }
